...
export default function ProfilePanel(state) {
  ...
  return `<div class="profile-screen" id="profile-screen"><section class="profile-panel" role="dialog" aria-modal="true" aria-labelledby="profile-title">
    ...
    ${signedIn ? `<div class="account-actions"><button class="primary-action account-action" id="sync-atlas-now" type="button" ${['checking', 'syncing'].includes(syncState) ? 'disabled' : ''}>${syncState === 'checking' ? 'Checking…' : syncState === 'syncing' ? 'Syncing…' : 'Sync now'}</button><button class="secondary-action account-action" id="sign-out-atlas" type="button">Sign out</button></div>` : `<form class="account-sign-in-form" id="atlas-sign-in-form"><label for="atlas-account-email">Sign up or log in with email</label><div><input id="atlas-account-email" name="email" type="email" autocomplete="email" placeholder="you@example.com" required ${account.status === 'loading' ? 'disabled' : ''}><button class="primary-action" type="submit" ${account.status === 'loading' || account.status === 'offline' ? 'disabled' : ''}>${account.status === 'loading' ? 'Connecting…' : 'Continue with email'}</button></div></form>`}
    ${signedIn ? `<form class="profile-name-form" id="profile-name-form"><label for="profile-display-name">Display name</label><div><input id="profile-display-name" name="displayName" maxlength="40" value="${escapeHtml(account.user.user_metadata?.display_name || '')}" placeholder="What should Atlas call you?"><button class="secondary-action" type="submit">Save name</button></div></form>` : ''}
